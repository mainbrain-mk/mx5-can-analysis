...
| CAN-Bus-Logging: Hardware/Pi-Infrastruktur, DBC-Pflege, Signal-Reverse-Engineering, TPMS, Renncockpit-GUI | [`status/can-bus.md`](status/can-bus.md) *(Stand 2026-09-26)* | [`logs/can-bus-status.md`](logs/can-bus-status.md) | `can_log_parser.py`, `can_byte_search.py`, `can_bitsearch.py`, `can_field_families.py`, `can_re_toolkit.py`, `can_field_segmentation.py`, `can_opendbc_crosscheck.py`, `obd_from_can.py`, `uds_did_sweep.py`, `tpms_poller.py`, `status_gui.py`, `dash_gui.py`, `session_logger.py` |
...
| CAN-Offline-Ausbeute (mehr aus vorhandenen Logs, ohne neue Fahrten) + Katalog offener Werte mit Fahrzeugtests | [`status/can-open-fields.md`](status/can-open-fields.md) *(Stand 2026-09-26)* | [`plans/can-offline-ausbeute-plan.md`](plans/can-offline-ausbeute-plan.md), `logs/can-bus-status.md` ("Offline-Ausbeute…") | `can_offline_lab.py`, `can_rare_bits.py`, `can_natural_events.py`, `can_anchor_sweep.py`, `can_field_inspect.py`, `can_open_fields.py` |
